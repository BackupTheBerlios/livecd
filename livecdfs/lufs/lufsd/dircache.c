...
search(struct dir_cache *cache, char *dir){
	...
	if(time(NULL) - d->d_stamp >= cache->ttl){
...
int
lu_cache_readdir(struct dir_cache *cache, char *dir, int offset, char *buf, int buflen){
    struct directory *d;
    struct direntry *de;
    struct list_head *p;
    int slen, res = -1;
