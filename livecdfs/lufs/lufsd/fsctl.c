...
get_filesystem(struct fs_ctl *ctl, char *fs){
    ...
    if(!(fops->init = (void*(*)(struct list_head*, struct dir_cache*, struct credentials*, void**))&livecdfs_init)){
	ERROR("dlerror()");
	goto fail_fops;
    }

    sprintf(buf, "%s_free", fs);
    if(!(fops->free = (void(*)(void*))&livecdfs_free)){
	ERROR("dlerror()");
	goto fail_fops;
    }

    sprintf(buf, "%s_mount", fs);
    if(!(fops->mount = (int(*)(void*))&livecdfs_mount)){
	ERROR("dlerror()");
	goto fail_fops;
    }

    sprintf(buf, "%s_umount", fs);
    if(!(fops->umount = (void(*)(void*))&livecdfs_umount))
	ERROR("dlerror()");

    sprintf(buf, "%s_readdir", fs);
    if(!(fops->readdir = (int(*)(void*, char*, struct directory*))&livecdfs_readdir))
	ERROR("dlerror()");

    sprintf(buf, "%s_stat", fs);
    if(!(fops->stat = (int(*)(void*, char*, struct lufs_fattr*))&livecdfs_stat))
	ERROR("dlerror()");

    sprintf(buf, "%s_mkdir", fs);
    if(!(fops->mkdir = (int(*)(void*, char*, int))&livecdfs_mkdir))
	ERROR("dlerror()");

    sprintf(buf, "%s_rmdir", fs);
    if(!(fops->rmdir = (int(*)(void*, char*))&livecdfs_rmdir))
	ERROR("dlerror()");

    sprintf(buf, "%s_create", fs);
    if(!(fops->create = (int(*)(void*, char*, int))&livecdfs_create))
	ERROR("dlerror()");

    sprintf(buf, "%s_unlink", fs);
    if(!(fops->unlink = (int(*)(void*, char*))&livecdfs_unlink))
	ERROR("dlerror()");

    sprintf(buf, "%s_rename", fs);
    if(!(fops->rename = (int(*)(void*, char*, char*))&livecdfs_rename))
	ERROR("dlerror()");

    sprintf(buf, "%s_open", fs);
    if(!(fops->open = (int(*)(void*, char*, unsigned))&livecdfs_open))
	ERROR("dlerror()");

    sprintf(buf, "%s_release", fs);
    if(!(fops->release = (int(*)(void*, char*))&livecdfs_release))
	ERROR("dlerror()");

    sprintf(buf, "%s_read", fs);
    if(!(fops->read = (int(*)(void*, char*, long long, unsigned long, char*))&livecdfs_read))
	ERROR("dlerror()");

    sprintf(buf, "%s_write", fs);
    if(!(fops->write = (int(*)(void*, char*, long long, unsigned long, char*))&livecdfs_write))
	ERROR("dlerror()");

    sprintf(buf, "%s_readlink", fs);
    if(!(fops->readlink = (int(*)(void*, char*, char*, int))&livecdfs_readlink))
	ERROR("dlerror()");

    sprintf(buf, "%s_link", fs);
    if(!(fops->link = (int(*)(void*, char*, char*))&livecdfs_link))
	ERROR("dlerror()");

    sprintf(buf, "%s_symlink", fs);
    if(!(fops->symlink = (int(*)(void*, char*, char*))&livecdfs_symlink))
	ERROR("dlerror()");

    sprintf(buf, "%s_setattr", fs);
    if(!(fops->setattr = (int(*)(void*, char*, struct lufs_fattr*))&livecdfs_setattr))
	ERROR("dlerror()");
